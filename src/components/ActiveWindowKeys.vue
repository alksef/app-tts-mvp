<template>
  <div class="key-display">
    <h3 class="display-title">Клавиши активного окна</h3>

    <div class="latest-key-container" v-if="store.hasLatestActiveWindowKey">
      <div class="latest-key">
        <span class="key-label">Последняя:</span>
        <span class="key-value">{{ store.latestActiveWindowKey?.key_name }}</span>
      </div>
    </div>

    <div class="no-key" v-else>
      <p>Клавиши еще не нажимались</p>
      <p class="hint">Нажмите любую клавишу, пока это окно активно</p>
    </div>

    <div class="keys-list" v-if="store.activeWindowKeys.length > 0">
      <div class="list-header">
        <span>Последние клавиши ({{ store.activeWindowKeys.length }})</span>
...
const displayKeys = computed(() => {
  return [...store.activeWindowKeys].reverse().slice(0, 10);
});
...
const handleClear = async () => {
  await store.clearActiveWindowKeys();
};
</script>
